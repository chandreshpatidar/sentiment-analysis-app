...
export const Slider = React.forwardRef<HTMLDivElement, SliderProps>(function Slider(props, ref) {
  const { marks: marksProp, label, showValue, hideThumb, ...rest } = props;
...
function SliderThumbs(props: { value?: number[]; hideThumb?: boolean }) {
  const { value } = props;
  return (
    <For each={value}>
      {(_, index) => (
        <ChakraSlider.Thumb
          key={index}
          index={index}
          display={props.hideThumb ? 'none' : undefined}
        >
